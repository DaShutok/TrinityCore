...
public:
    ...
    void ValidateScriptHookVersion() //May be Deprecated in the future as this would be bundled as a patch.
    {
        ...
        {
            sLog->outError(LOG_FILTER_VAS_AUTOBALANCE, "  VAS_Script_Hooks v%4.2f Found", sScriptMgr->VAS_Script_Hooks());
            sLog->outError(LOG_FILTER_VAS_AUTOBALANCE, "  VAS AutoBalance needs %4.2f+ to run correctly!", VAS_SCRIPT_HOOK_VERSION_NEEDED);
...
class VAS_AutoBalance_WorldScript : public WorldScript
{
    ...
    void SetInitialWorldSettings()
    {
        ...
        sLog->outDebug(LOG_FILTER_VAS_AUTOBALANCE, "----------------------------------------------------");
        sLog->outDebug(LOG_FILTER_VAS_AUTOBALANCE, "  Powered by {VAS} AutoBalance v%4.2f.%u ", AutoBalanceConfig::Instanace()->GetFloatValue(VAS_VERSION), AutoBalanceConfig::Instanace()->GetIntValue(VAS_SUB_VERSION));
        sLog->outDebug(LOG_FILTER_VAS_AUTOBALANCE, "----------------------------------------------------");
        AutoBalanceConfig::Instanace()->ValidateScriptHookVersion();
        sLog->outDebug(LOG_FILTER_VAS_AUTOBALANCE, "  xPlayer = %4.1f ", AutoBalanceConfig::Instanace()->GetFloatValue(CONFIG_XPLAYER));
        sLog->outDebug(LOG_FILTER_VAS_AUTOBALANCE, "  AutoInstance = %u ", AutoBalanceConfig::Instanace()->GetIntValue(AUTOINSTANCE));
        sLog->outDebug(LOG_FILTER_VAS_AUTOBALANCE, "  PlayerChangeNotify = %u ", AutoBalanceConfig::Instanace()->GetIntValue(PLAYER_CHANGE_NOTIFY));
        sLog->outDebug(LOG_FILTER_VAS_AUTOBALANCE, "  Min.D.Mod = %4.2f ", AutoBalanceConfig::Instanace()->GetFloatValue(MIN_D_MOD));
        sLog->outDebug(LOG_FILTER_VAS_AUTOBALANCE, "  Min.HP.Mod = %4.2f ", AutoBalanceConfig::Instanace()->GetFloatValue(MIN_HP_MOD));
        sLog->outDebug(LOG_FILTER_VAS_AUTOBALANCE, "  VAS.Group.Modifer = %4.2f ", AutoBalanceConfig::Instanace()->GetFloatValue(VAS_GROUP_MODIFIER));
        sLog->outDebug(LOG_FILTER_VAS_AUTOBALANCE, "  VAS.Damage.Modifer = %4.2f ", AutoBalanceConfig::Instanace()->GetFloatValue(VAS_DAMAGE_MODIFIER));
        sLog->outDebug(LOG_FILTER_VAS_AUTOBALANCE, "  VasDebug   =  %u ", AutoBalanceConfig::Instanace()->GetIntValue(VAS_DEBUG));
        sLog->outDebug(LOG_FILTER_VAS_AUTOBALANCE, "  DebugByID   =  %u", AutoBalanceConfig::Instanace()->GetIntValue(VAS_DEBUG_BY_ID));
        sLog->outDebug(LOG_FILTER_VAS_AUTOBALANCE, "  Creature_Update_Timer   =  %u", AutoBalanceConfig::Instanace()->GetIntValue(CREATURE_UPDATE_TIMER));
        sLog->outDebug(LOG_FILTER_VAS_AUTOBALANCE, "----------------------------------------------------\n");
    }
...
class VAS_AutoBalance_PlayerScript : public PlayerScript
{
    ...
    void OnLogin(Player *Player)
    {
        ...
        if (AutoBalanceConfig::Instanace()->GetIntValue(VAS_DEBUG) >= 3)
            sLog->outDebug(LOG_FILTER_VAS_AUTOBALANCE, "### VAS_AutoBalance_PlayerScript - OnLogin Player=%s", Player->GetName());
    }
};
...
class VAS_AutoBalance_UnitScript : public UnitScript
{
    ...
    uint32 DealDamage(Unit* AttackerUnit, Unit *playerVictim,uint32 damage, DamageEffectType damagetype)
    {
        ...
        if (AttackerUnit->GetMap()->IsDungeon() && playerVictim->GetMap()->IsDungeon())
            ...
            {
                if (AutoBalanceConfig::Instanace()->GetIntValue(VAS_DEBUG) >= 3)
                    sLog->outDebug(LOG_FILTER_VAS_AUTOBALANCE, "### VAS_AutoBalance_UnitScript - VAS_Unit_DealDamage Attacker=%s Victim=%s Start Damage=%u",AttackerUnit->GetName(),playerVictim->GetName(),damage);
                damage = VAS_Modifer_DealDamage(AttackerUnit,damage);
                if (AutoBalanceConfig::Instanace()->GetIntValue(VAS_DEBUG) >= 3)
                    sLog->outDebug(LOG_FILTER_VAS_AUTOBALANCE, "### VAS_AutoBalance_UnitScript - VAS_Unit_DealDamage Attacker=%s Victim=%s End Damage=%u",AttackerUnit->GetName(),playerVictim->GetName(),damage);
            }
            return damage;
    }
    ...
    void CalculateSpellDamageTaken(SpellNonMeleeDamage *damageInfo, int32 damage, SpellEntry const *spellInfo, WeaponAttackType attackType, bool crit)
    {
        ...
        if ((damageInfo->attacker->GetMap()->IsDungeon() && damageInfo->target->GetMap()->IsDungeon()) || ( damageInfo->attacker->GetMap()->IsBattleground() && damageInfo->target->GetMap()->IsBattleground()))
            ...
            {
                if (AutoBalanceConfig::Instanace()->GetIntValue(VAS_DEBUG) >= 3)
                    sLog->outDebug(LOG_FILTER_VAS_AUTOBALANCE, "### VAS_AutoBalance_UnitScript - CalculateSpellDamageTaken Attacker=%s Victim=%s Start Damage=%u",damageInfo->attacker->GetName(),damageInfo->target->GetName(),damageInfo->damage);
                ...
                if (AutoBalanceConfig::Instanace()->GetIntValue(VAS_DEBUG) >= 3)
                    sLog->outDebug(LOG_FILTER_VAS_AUTOBALANCE, "### VAS_AutoBalance_UnitScript - CalculateSpellDamageTaken Attacker=%s Victim=%s End Damage=%u",damageInfo->attacker->GetName(),damageInfo->target->GetName(),damageInfo->damage);
            }
            return;
    }
    ...
    void CalculateMeleeDamage(Unit *playerVictim, uint32 damage, CalcDamageInfo *damageInfo, WeaponAttackType attackType)
    {
        ...
        if (((damageInfo->attacker->GetMap()->IsDungeon() && damageInfo->target->GetMap()->IsDungeon()) || (damageInfo->attacker->GetMap()->IsBattleground() && damageInfo->target->GetMap()->IsBattleground())) && (damageInfo->attacker->GetTypeId() != TYPEID_PLAYER))
            ...
            {
                if (AutoBalanceConfig::Instanace()->GetIntValue(VAS_DEBUG) >= 3)
                    sLog->outDebug(LOG_FILTER_VAS_AUTOBALANCE, "### VAS_AutoBalance_UnitScript - CalculateMeleeDamage Attacker=%s Victim=%s Start Damage=%u",damageInfo->attacker->GetName(),damageInfo->target->GetName(),damageInfo->damage);

                damageInfo->damage *= CreatureInfo[damageInfo->attacker->GetGUID()].DamageMultiplier;

                if (AutoBalanceConfig::Instanace()->GetIntValue(VAS_DEBUG) >= 3)
                    sLog->outDebug(LOG_FILTER_VAS_AUTOBALANCE, "### VAS_AutoBalance_UnitScript - CalculateMeleeDamage Attacker=%s Victim=%s End Damage=%u",damageInfo->attacker->GetName(),damageInfo->target->GetName(),damageInfo->damage);
            }
            return;
    }
...
class VAS_AutoBalance_AllMapScript : public AllMapScript
{
    ...
    void OnPlayerEnterAll(Map* map, Player* player)
    {
        ...
        {
            sLog->outDebug(LOG_FILTER_VAS_AUTOBALANCE, "----------------------------------------------------");
            sLog->outDebug(LOG_FILTER_VAS_AUTOBALANCE, "## VAS_AutoBalance_AllMapScript - OnPlayerEnterAll");
            sLog->outDebug(LOG_FILTER_VAS_AUTOBALANCE, "## For InsatanceID %u",map->GetInstanceId());
            sLog->outDebug(LOG_FILTER_VAS_AUTOBALANCE, "## IsDungeon= %u",map->GetEntry()->IsDungeon());
            sLog->outDebug(LOG_FILTER_VAS_AUTOBALANCE, "## For Map %u",player->GetMapId());
            sLog->outDebug(LOG_FILTER_VAS_AUTOBALANCE, "## PlayersInMap %u",map->GetPlayersCountExceptGMs());
            sLog->outDebug(LOG_FILTER_VAS_AUTOBALANCE, "## pDifficulty %u",uint32(player->GetDifficulty(player->GetMap()->IsHeroic())));
            sLog->outDebug(LOG_FILTER_VAS_AUTOBALANCE, "## pGetDungeonDifficulty %u",uint32(player->GetDungeonDifficulty()));
            sLog->outDebug(LOG_FILTER_VAS_AUTOBALANCE, "## pGetRaidDifficulty %u",uint32(player->GetRaidDifficulty()));
            sLog->outDebug(LOG_FILTER_VAS_AUTOBALANCE, "## maxPlayers %u",((InstanceMap*)sMapMgr->FindMap(player->GetMapId(), player->GetInstanceId()))->GetMaxPlayers());
            sLog->outDebug(LOG_FILTER_VAS_AUTOBALANCE, "## IsHeroic=%s IsRaid=%s IsRegularDifficulty=%s IsRaidOrHeroicDungeon=%s IsNonRaidDungeon=%s",BOOL_TO_STRING(player->GetMap()->IsHeroic()),BOOL_TO_STRING(player->GetMap()->IsRaid()),BOOL_TO_STRING(player->GetMap()->IsRegularDifficulty()),BOOL_TO_STRING(player->GetMap()->IsRaidOrHeroicDungeon()),BOOL_TO_STRING(player->GetMap()->IsNonRaidDungeon()));
            sLog->outDebug(LOG_FILTER_VAS_AUTOBALANCE, "----------------------------------------------------\n");
        }
    ...
    void OnPlayerLeaveAll(Map* map, Player* player)
    {
        ...
        if (instancePlayerCount >=1)
        {
            ...
            {
                sLog->outDebug(LOG_FILTER_VAS_AUTOBALANCE, "----------------------------------------------------");
                sLog->outDebug(LOG_FILTER_VAS_AUTOBALANCE, "## VAS_AutoBalance_AllMapScript - OnPlayerLeaveAll");
                sLog->outDebug(LOG_FILTER_VAS_AUTOBALANCE, "## For InsatanceID %u",map->GetInstanceId());
                sLog->outDebug(LOG_FILTER_VAS_AUTOBALANCE, "## IsDungeon= %u",map->GetEntry()->IsDungeon());
                sLog->outDebug(LOG_FILTER_VAS_AUTOBALANCE, "## For Map %u",player->GetMapId());
                sLog->outDebug(LOG_FILTER_VAS_AUTOBALANCE, "## PlayersInMap %u",instancePlayerCount);
                sLog->outDebug(LOG_FILTER_VAS_AUTOBALANCE, "----------------------------------------------------\n");
            }
...
class VAS_AutoBalance_WorldMapScript : public WorldMapScript
{
    ...
    void OnPlayerEnter(Map* map, Player* player)
    {
        ...
        if (AutoBalanceConfig::Instanace()->GetIntValue(VAS_DEBUG) >= 3)
            sLog->outDebug(LOG_FILTER_VAS_AUTOBALANCE, "### VAS_AutoBalance_WorldMapScript - OnPlayerEnter Map=%s player=%s",map->GetMapName(),player->GetName());
    }
    ...
    void OnPlayerLeave(Map* map, Player* player)
    {
        ...
        if (AutoBalanceConfig::Instanace()->GetIntValue(VAS_DEBUG) >= 3)
            sLog->outDebug(LOG_FILTER_VAS_AUTOBALANCE, "### VAS_AutoBalance_WorldMapScript - OnPlayerLeave Map=%s player=%s",map->GetMapName(),player->GetName());
    }
};
...
class VAS_AutoBalance_AllCreatureScript : public AllCreatureScript
{
    ...
    void ModifyCreatureAttributes(Creature* creature)
    {
        ...
        if((AutoBalanceConfig::Instanace()->GetIntValue(VAS_DEBUG) >= 3))
        {
            sLog->outDebug(LOG_FILTER_VAS_AUTOBALANCE, "## VAS-AutoBalance MobID=%u MapID=%u creatureName=%s  GUID=%llu  instancePlayerCount=%u", creatureTemplate->Entry, creature->GetMapId(), creatureTemplate->Name.c_str(), creature->GetGUID(), instancePlayerCount);
            sLog->outDebug(LOG_FILTER_VAS_AUTOBALANCE, "## VAS-AutoBalance MapDifficulty=%u Health=%u / %u healthMultiplier=%4.5f VAS_Group_Modifer=%4.2f", creature->GetMap()->GetDifficulty(), scaledHealth, baseHealth, healthMultiplier, AutoBalanceConfig::Instanace()->GetFloatValue(VAS_GROUP_MODIFIER));
            sLog->outDebug(LOG_FILTER_VAS_AUTOBALANCE, "## VAS-AutoBalance maxNumberOfPlayers=%u IsRaid=%s", maxNumberOfPlayers, BOOL_TO_STRING(creature->GetMap()->IsRaid()));
            sLog->outDebug(LOG_FILTER_VAS_AUTOBALANCE, "## VAS-AutoBalance Mana %u / %u", baseMana, scaledMana);
            sLog->outDebug(LOG_FILTER_VAS_AUTOBALANCE, "## VAS-AutoBalance damageMultiplier=%4.2f", damageMultiplier);
        }
