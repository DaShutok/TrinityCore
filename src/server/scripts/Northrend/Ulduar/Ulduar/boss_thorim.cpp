...
#define EMOTE_BARRIER                           "!Coloso rúnico se rodea de una chisporroteante Barrera Rúnica!"
#define EMOTE_MIGHT                             "¡Gigante runa antigua fortifica a los aliados cercanos con poder rúnico!"
...
public:
    ...
    struct npc_thorim_arena_phaseAI : public ScriptedAI
    {
        npc_thorim_arena_phaseAI(Creature* pCreature) : ScriptedAI(pCreature)
        {
            ...
            if(IsInArena)
                me->GetMotionMaster()->MovePoint(id,2134.0f,-262.0f,419.84f);
        }
            ...
            if (me->getThreatManager().isThreatListEmpty())
            {
                ...
                if (pMap->IsDungeon())
                {
                    ...
                    if (!PlayerList.isEmpty())
                    {
                        ...
                        {
                            if (i->getSource() && i->getSource()->isAlive() && !i->getSource()->isGameMaster() && isOnSameSide(i->getSource()))
...
class spell_charge_orb: public SpellScriptLoader
{
public:
    spell_charge_orb(): SpellScriptLoader("spell_charge_orb") { }

    class spell_charge_orb_SpellScript: public SpellScript
    {
        PrepareSpellScript(spell_charge_orb_SpellScript);
        ...
        void Register()
        {
            OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(spell_charge_orb_SpellScript::SelectTarget, EFFECT_0, TARGET_UNIT_SRC_AREA_ENTRY);
        }
    };

    SpellScript* GetSpellScript() const
    {
        return new spell_charge_orb_SpellScript();
    }
};
