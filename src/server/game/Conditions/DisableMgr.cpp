    ...
    do
    {
        ...
        switch (type)
        {
            ...
            case DISABLE_TYPE_MMAP:
            {
                ...
                if (!mapEntry)
                {
                    sLog->outError(LOG_FILTER_MAPS, "Pathfinding for map entry %u from `disables` doesn't exist in dbc, skipped.", entry);
                    continue;
                }
                if (flags)
                    sLog->outError(LOG_FILTER_MAPS, "Disable flags specified for pathfinding in map %u, useless data.", entry);

                sLog->outInfo(LOG_FILTER_MAPS, "Pathfinding disabled for map %u.", entry);
                break;
            }
            default:
                break;
